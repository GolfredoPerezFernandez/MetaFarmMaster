...
//
function parseNFTStatus(NFTs, marketItems) {
  ...
  let result = [];
  NFTs.map((n) => {
	console.log(n.token_address)
    let r = marketItems.slice().reverse().find(
      (e) =>
        e.nftContract === n?.token_address &&
        e.tokenId === n?.token_id &&
        e.confirmed === true
    );
...
console.log(account)
console.log(owner)
let isowner=account.toString().toLowerCase() === owner.toString().toLowerCase() ?true:false
    result.push({ ...n, ...r, isNFTOwner: isowner });
  });
  return result;
}
...
const options3333 = {
	chain: "0x61",
	token_address: "0xe990eAA4D078f3F3018F692A5880423cF9536f92",
  };
const polygonNFTs = await Moralis.Web3API.account.getNFTsForContract(options3333);
	const createdMarketItems = polygonNFTs.result;
	const CreatedMarketItems = createdMarketItems;

	const NFTsWithMetadataParsed = parseNFTStatus(NFTsWithMetadata, CreatedMarketItems);

	$('.p').animate({top:'300',opacity:'0'}, 3000,
    $.bez([0.19, 1, 0.22, 1]));
    $("#preloader").animate({top: '-5000'}, 2000, $.bez([0.19, 1, 0.22, 1]));
...
const metadataToPopup = (nft) => {
	console.log('nft '+JSON.stringify(nft))
  let metadata = nft.metadata;
  let buttonText = nft.sold ? 'Visit page' : 'Visit';
  let price = nft.sold || nft.price === undefined ? '' : `<div><b>Price:</b> ${nft.price / ("1e" + 18)} BNB</div>`;
  let image = `<img style="width:300px;" src="${metadata?.image}"/>`;
  return `<div style="display:flex;flex-direction:column"> ${image} <b>${metadata?.name}</b> <div>${metadata?.description}</div> <div><b>Owner:</b> ${slicedWallet(nft.owner)}</div> ${price} <button onclick="window.open('${FRONTEND_BASE_URL}nft/${nft.token_id}','_blank')">${buttonText}</button></div>`;
};

const calculateColor = (inventoryItem) => {
	console.log( inventoryItem )

  let color = inventoryItem.isNFTOwner ? '#DEEBFF' :
    inventoryItem.sold ? '#000000' : '#336699';
  return color;
}

const getInventoryItemById = (inventory, inventoryItemId) => {
	console.log(" result "+JSON.stringify(inventory))
	console.log(" result "+inventoryItemId)
